...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while curr:
            ...
            if curr.random is not None and not randomNodeToDependents.get(curr.random):
            ...
            randomNode = randomNodeToDependents[curr.random]["newCopy"] if curr.random is not None else None

            copy = Node(curr.val, None, randomNode)
            # we haven't seen this node yet and it's not initialiezd
            if not randomNode and curr.random is not None:
